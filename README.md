...
│   │       ├── fundamental1.md      # prompt/enigmas do 4º ao 5º ano
...
1. **`/` — [SelecaoAno.jsx](frontend/src/pages/SelecaoAno.jsx)**
   Tela inicial. O aluno escolhe o ano/série (define `ano.turmaKey`, que é a chave usada pelo backend para escolher o prompt certo). Se já existir uma partida em andamento salva no navegador, redireciona automaticamente para a tela certa (chat, resultado ou desclassificado).
...
3. **`/equipe` — [SelecaoEquipe.jsx](frontend/src/pages/SelecaoEquipe.jsx)**
   O aluno escolhe a cor da equipe. Ao escolher, `setEquipe()` (em [useAppStore.js](frontend/src/store/useAppStore.js)) gera um novo `sessionId` (UUID), zera as mensagens e marca `startedAt` — ou seja, **é aqui que o cronômetro da partida começa**.
...
5. Quando a IA detecta que **todos os enigmas foram resolvidos**, ela inclui um marcador técnico invisível no início da resposta final. O backend detecta esse marcador no meio do streaming e avisa o frontend via evento `{"type":"done","completed":true}`. O frontend então:
   - marca `finishedAt` (`finish()` na store);
   - espera ~1.8s (para o aluno terminar de ler a mensagem de vitória);
   - navega para **`/resultado` — [Resultado.jsx](frontend/src/pages/Resultado.jsx)**, que mostra o tempo total (em minutos) e um botão **REINICIAR** (que limpa a store e volta para `/`).

6. Se o aluno sair da aba/minimizar por tempo demais, é redirecionado para **`/desclassificado` — [Desclassificado.jsx](frontend/src/pages/Desclassificado.jsx)**, também com opção de reiniciar.
...
- `stream_chat_response()` implementa uma pequena máquina de estados (`deciding` → `strip_newline` → `streaming`) para detectar o marcador `[[JOGO_CONCLUIDO]]` **mesmo que ele chegue picado em vários pedacinhos** do streaming da OpenAI (a API não garante o tamanho de cada chunk). O marcador nunca é enviado ao frontend — só o texto de "parabéns" que vem depois dele.
...
Pré-requisitos: **Python 3.11+** e **Node.js 20+** instalados. Os comandos abaixo assumem PowerShell no Windows, a partir da raiz do repositório (`Integra2026/`).
...
Depois de editar um `.md`, reinicie o backend (ou deixe o `--reload` do uvicorn recarregar sozinho) para a mudança ter efeito — os prompts são carregados uma vez na inicialização (`PROMPTS` em `prompts/__init__.py`).
...
CREATE TABLE messages (
    ...
    turma TEXT NOT NULL,        -- ex: "fundamental1", "medio_tecnico"
